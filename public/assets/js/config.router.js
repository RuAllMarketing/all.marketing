...
function ($stateProvider, $urlRouterProvider, $controllerProvider, $compileProvider, $filterProvider, $provide, $ocLazyLoadProvider, jsRequires) {
    ...
    $urlRouterProvider.otherwise("/landing-page/welcome");
    //
    // Set up the states
    $stateProvider.state('app', {
        ...
        title: 'Dashboard',
        resolve: loadSequence('propertyCtrl', 'angularFileUpload', 'uploadCtrl'),
        ncyBreadcrumb: {
            label: 'Dashboard'
        }
	...
	.state('login', {
        ...
        controller: function($scope){
            ...
            $scope.update = function(user){
                ...
                setTimeout(function(){
                    $.post($scope.$root.ajaxUrls['user-login'], { 'email': user.email, 'password': user.password }, function(response){
                        ...
                        if(response.status == 'ok'){
                            ...
                            Cookies.set('user', $scope.$root.user);
        ...
        controller: function($scope){
            ...
            $.getJSON($scope.$root.ajaxUrls['user-activate'].replace('%hash%', hash), function(response){
                if(response.status == 'ok'){
                    $scope.$root.user = {
                        name: response.name,
                        email: response.email,
                    ...
                    alert('Problem occured during activation');
                }
            });
        },
	    templateUrl: "assets/views/login_activate.html"
	}).state('login.forgot', {
	    url: '/forgot',
        ...
        controller: function($scope){
            ...
            $scope.update = function(user){
                $scope.errorMessage = false;
                setTimeout(function(){
                    $.post($scope.$root.ajaxUrls['user-register'], $scope.user, function(response){
                        response = JSON.parse(response);

                        if(response.status == 'ok'){
                            $scope.finishedRegistration = true;
                            $scope.$apply();
                        }else{
	...
	.state('landing', {
	    ...
	    url: '/welcome',
        resolve: loadSequence('propertyDisplay'),
	    templateUrl: "assets/views/landing_page.html"
	}).state('landing.view-item', {
        url: '/view-item/:id',
        controller: function($scope){
            setTimeout(function(){
                var parts = location.hash.split('/');
                ...
                $.getJSON($scope.$root.ajaxUrls['list-top-property'], function(response){
                    ...
                    if(property){
                        ...
                        location.hash = '#/landing-page/welcome';
                    }
                });
            }, 1000);
        },
	    templateUrl: "assets/views/landing_page_item.html"
	});
